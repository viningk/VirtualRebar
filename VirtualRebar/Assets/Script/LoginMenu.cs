...
public class LoginMenu : MonoBehaviour
{
    ...
    public InputField Register_FirstNameField;
    public InputField Register_LastNameField;
    public InputField Register_CompanyField;
    ...
    IEnumerator LoginUser()
    {
        ...
        if (response == "Success")
        {
            //Username and Password were correct. Stop showing 'Loading...' 
            ResetAllUIElements();
            loadingParent.gameObject.SetActive(false);
            UserAccountManager.UACInst.LogIn(playerUsername, playerPassword);
        }
    ...
    public void Register_RegisterButtonPressed()
    {
        ...
        string FirstName = Register_FirstNameField.text;
        string LastName = Register_LastNameField.text;
        string Company = Register_CompanyField.text;
        ...
        if (playerUsername.Length > 3)
        {
            if (FirstName != "" && LastName != "")
            {
                if (Company != "")
                {
                    if (playerPassword.Length > 5)
                    {
                        ...
                        if (playerPassword == confirmedPassword)
                        {
                            ...
                            StartCoroutine(RegisterUser(data));
                        }
                        else
                        {
                ...
                {
                    //Password too short so show error
                    Register_ErrorText.text = "Error: Company name not filled in";
                }
            }
            else
            {
                //Password too short so show error
                Register_ErrorText.text = "Error: First or Last name not filled in";
            }
        }
        else
        {
    ...
    IEnumerator RegisterUser(string data)
    {
        IEnumerator e = DCF.RegisterUser(playerUsername, playerPassword, data); // << Send request to register a new user, providing submitted username and password. It also provides an initial value for the data string on the account, which is "Hello World".
        while (e.MoveNext())
        {
            yield return e.Current;
        }
